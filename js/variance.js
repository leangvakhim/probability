...
const steps = [
    {
        ...
        description: `
            <p class="mb-4">Imagine you run a specialty coffee stand. Let <span class="font-bold text-blue-600">\\(X\\)</span> be the number of "Signature Lattes" you sell in a random 10-minute window.</p>
            <p class="mb-4">Based on weeks of tracking, you know you usually sell between 0 and 3 lattes. The probabilities of selling exactly <span class="font-bold text-blue-600">\\(x\\)</span> lattes, denoted as <span class="font-bold text-blue-600">\\(P(x)\\)</span>, are shown in the table.</p>
            <p class="text-sm text-slate-500 bg-white p-3 rounded border-l-4 border-blue-400">Notice that all probabilities add up to exactly 1.0 (100%). This is our discrete probability distribution.</p>
        `,
        tableCols: ['x', 'P'],
        showFormulas: false
    },
    {
        ...
        description: `
            ...
            <p class="mb-4">The <strong>Expected Value</strong>, denoted as <span class="font-bold text-blue-600">\\(\\mu\\)</span> (mu) or <span class="font-bold text-blue-600">\\(E(X)\\)</span>, is the long-term average.</p>
            ...
            <p class="text-sm font-semibold text-green-700 bg-green-50 p-3 rounded">\\( \\mu = 1.6 \\) lattes.</p>
        `,
        tableCols: ['x', 'P', 'xP'],
        showFormulas: false
    },
    {
        ...
        description: `
            ...
            <p class="mb-4">We find this by calculating the <strong>Deviation</strong>: <span class="font-bold text-blue-600">\\((x - \\mu)\\)</span>.</p>
            ...
            <p class="text-sm text-slate-500 bg-white p-3 rounded border-l-4 border-yellow-400">Note: If we just added these deviations up, the positives and negatives would cancel each other out!</p>
        `,
        tableCols: ['x', 'P', 'Dev'],
        showFormulas: false
    },
    {
        ...
        description: `
            <p class="mb-4">To solve the problem of negatives cancelling out positives, we <strong>square</strong> each deviation: <span class="font-bold text-blue-600">\\((x - \\mu)^2\\)</span>.</p>
    ...
    },
    {
        ...
        description: `
            ...
            <p class="mb-4">Therefore, we must multiply each squared deviation by its probability: <span class="font-bold text-blue-600">\\((x - \\mu)^2 \\cdot P(x)\\)</span>.</p>
    ...
    },
    {
        ...
        description: `
            <p class="mb-4">We made it! To find the final <strong>Variance</strong>, denoted as <span class="font-bold text-blue-600">\\(Var(X)\\)</span> or <span class="font-bold text-blue-600">\\(\\sigma^2\\)</span>, we simply sum up the probability-weighted squared deviations.</p>
            <p class="mb-4"><strong>Variance = 0.84</strong></p>
            <p class="text-sm bg-blue-50 p-3 rounded mt-4">Variance is a single number that tells us how "volatile" or spread out our coffee sales are. A larger variance means less predictability.</p>
        `,
...
function init() {
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        dot.className = `w-2.5 h-2.5 rounded-full transition-colors ${i === 0 ? 'bg-blue-600' : 'bg-slate-300'}`;
        dot.id = `dot-${i}`;
...
function changeStep(direction) {
    ...
    // Re-trigger animation by removing class, reflowing, and re-adding class
    // This is safer than replacing the DOM nodes which breaks our const references
...
function renderStep() {
    ...
    data.forEach((row, index) => {
        ...
        tableHTML += `<td class="p-3 border-r border-slate-100">\\( ${row.x} \\)</td>`;
        tableHTML += `<td class="p-3 border-r border-slate-100">\\( ${row.p} \\)</td>`;

        if (step.tableCols.includes('xP')) {
            const isLast = step.tableCols[step.tableCols.length - 1] === 'xP';
            tableHTML += `<td class="p-3 border-r border-slate-100 ${isLast ? 'highlight-col' : ''}">\\( ${row.xp.toFixed(1)} \\)</td>`;
        }
        ...
        if (step.tableCols.includes('SqDev')) {
            ...
            tableHTML += `<td class="p-3 border-r border-slate-100 ${isLast ? 'highlight-col' : ''}">\\( ${row.sqdev.toFixed(2)} \\)</td>`;
        }
        if (step.tableCols.includes('Weighted')) {
            const isLast = step.tableCols[step.tableCols.length - 1] === 'Weighted';
            tableHTML += `<td class="p-3 ${isLast ? 'highlight-col' : ''}">\\( ${row.weighted.toFixed(3)} \\)</td>`;
        }
    ...
    tableHTML += `<td class="p-3 text-right">Sum (\\(\\Sigma\\)) =</td>`;
    tableHTML += `<td class="p-3">\\( 1.0 \\)</td>`;
